...
namespace Cryptonite.ECDH.Core.Op
{
    internal static class MontgomeryOperations
    {
        ...
        /// <returns></returns>
        internal static Span<byte> ScalarMultiplication(Span<byte> n, Span<byte> p, int qSize)
        {
            ...
            return new Span<byte>(q);
        }

        private static FieldElement CalculateLadderStep(Span<byte> n, ref FieldElement p, int noffset = 0)
        {
            var e = new byte[32];
            uint i;
            FieldElement x1;
